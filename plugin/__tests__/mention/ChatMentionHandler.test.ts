...
describe('ChatMentionHandler', () => {
    ...
    beforeEach(() => {
        ...
        handler = new ChatMentionHandler(mockApp, mockDecorator);
        mockInput = document.createElement('div');
        handler.attachToInput(mockInput);
    });

    test('should NOT delegate agent mention clicks (agents handled differently in chat)', () => {
        ...
        // Agent clicks should NOT delegate to decorator (handled specially in chat)
        expect(mockDecorator.handleMentionClick).not.toHaveBeenCalled();
    });

    test('should stop propagation on agent clicks', () => {
        // Create an agent token
        const token = document.createElement('span');
        token.className = 'spark-token spark-token-agent';
        token.setAttribute('data-type', 'agent');
