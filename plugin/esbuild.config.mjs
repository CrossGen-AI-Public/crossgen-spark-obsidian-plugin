...
import { copyFileSync, writeFileSync, watch, statSync } from "fs";
...
if (prod) {
    ...
    // Watch styles.css for changes (with debounce to handle spurious macOS fs.watch events)
    let lastMtime = statSync("styles.css").mtimeMs;
    watch("styles.css", (eventType) => {
        if (eventType === "change") {
            const currentMtime = statSync("styles.css").mtimeMs;
