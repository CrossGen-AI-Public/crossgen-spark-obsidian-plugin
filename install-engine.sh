...
# Spark Engine Installer
# Downloads pre-built engine from GitHub Releases
...
echo -e "${BLUE}╔════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║   Spark Engine Installation            ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════╝${NC}"
echo ""
echo -e "${BLUE}This script installs the Spark engine only.${NC}"
...
if [ "$VERSION" = "latest" ]; then
    echo -e "${YELLOW}→ Fetching latest engine version...${NC}"
    # Get latest engine release (tags start with 'engine-')
    RELEASES_API="https://api.github.com/repos/CrossGen-AI-Public/crossgen-spark-obsidian-plugin/releases"
    if [ "$DOWNLOAD_TOOL" = "curl" ]; then
        LATEST_TAG=$(curl -fsSL "$RELEASES_API" | grep -o '"tag_name": "engine-[^"]*"' | head -1 | cut -d'"' -f4)
    else
        LATEST_TAG=$(wget -qO- "$RELEASES_API" | grep -o '"tag_name": "engine-[^"]*"' | head -1 | cut -d'"' -f4)
    fi
    if [ -z "$LATEST_TAG" ]; then
        echo -e "${RED}✗ Could not find latest engine release${NC}"
        echo "  Check available releases: $REPO_URL/releases"
        exit 1
    fi
    VERSION=$(echo "$LATEST_TAG" | sed 's/^engine-//')
    echo -e "${GREEN}✓ Latest engine version: ${VERSION}${NC}"
fi

TARBALL_NAME="spark-engine-v${VERSION}.tar.gz"
DOWNLOAD_URL="$REPO_URL/releases/download/engine-${VERSION}/${TARBALL_NAME}"

# Download and extract
echo -e "${YELLOW}→ Downloading Spark engine v${VERSION}...${NC}"
...
if ! $DOWNLOAD_CMD "$DOWNLOAD_URL" | tar -xz -C "$INSTALL_DIR" --strip-components=1; then
    echo -e "${RED}✗ Failed to download engine${NC}"
    echo ""
    echo "Possible causes:"
    echo "  • Engine release engine-${VERSION} may not exist"
    echo "  • Network connectivity issues"
    echo "  • GitHub rate limiting"
    echo ""
    echo "Try:"
    echo "  • Check available releases: $REPO_URL/releases"
    echo "  • Specify a version: SPARK_VERSION=0.2.5 bash install-engine.sh"
    exit 1
fi
...
if [[ ":$PATH:" != *":$BIN_DIR:"* ]]; then
    if [ -n "$SHELL_PROFILE" ]; then
        if ! grep -q "$BIN_DIR" "$SHELL_PROFILE" 2>/dev/null; then
            ...
            echo "" >> "$SHELL_PROFILE"
            echo "# Spark engine (added by installer)" >> "$SHELL_PROFILE"
...
echo ""
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${GREEN}✓ Spark engine installation complete!${NC}"
echo ""
...
echo -e "${YELLOW}Start the engine:${NC}"
