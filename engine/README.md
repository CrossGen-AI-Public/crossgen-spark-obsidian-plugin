...
The intelligence layer for Spark Assistant. A Node.js engine that watches your Obsidian vault, parses Spark syntax, loads context, calls Claude API, and writes results back to files.
...
│   ├── main.ts        # Main engine orchestrator
│   ├── cli/                  # CLI utilities
│   │   ├── registry.ts       # Global engine registry
│   │   └── EngineInspector.ts # State inspection
...
```bash
# One-command engine installation
...
- Builds and installs the engine globally
...
```bash
# Engine control
spark start [vault-path]              # Start engine (foreground)
...
```bash
# Start engine in foreground (logs to console)
spark start ~/Documents/Obsidian --debug

# Start engine in background (simple)
spark start ~/Documents/Obsidian &

# Start engine in background (persistent, with logs)
nohup spark start ~/Documents/Obsidian > ~/.spark/engine.log 2>&1 &

# Check all running engines
spark status
# Output:
...
For rapid development iteration, use **tsx watch** which automatically restarts the engine when source files change:

```bash
# Start dev mode from engine directory (watches all TypeScript files)
cd engine
npm run dev
...
# ✓ Restart engine process with new code
...
- Engine watches `.spark/config.yaml` in your vault
- Reloads config automatically when file changes
- No engine restart needed for config changes
...
# → Engine automatically restarts with new code
...
The engine maintains a global registry at `~/.spark/registry.json` that tracks all running engines. This allows:
- `spark status` to show all running engines across different vaults
- Detection of already-running engines to prevent duplicates
...
- CLI tests (4 tests) - registry, EngineInspector, cli

## TypeScript Configuration

The engine uses two TypeScript configurations:
...
- **[Engine Spec](../specs/ENGINE_SPEC.md)** - Detailed engine specification
