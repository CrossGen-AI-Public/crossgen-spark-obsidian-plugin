...
describe('ConfigLoader', () => {
    ...
    describe('load', () => {
        ...
        it('should merge user config with defaults', async () => {
            ...
            // User overrides
            expect(config.engine.debounce_ms).toBe(500);
            expect(config.logging.level).toBe('debug');

            // Defaults preserved
            expect(config.engine.watch.patterns).toEqual(DEFAULT_SPARK_CONFIG.engine.watch.patterns);
        ...
        it('should completely replace arrays', async () => {
            ...
            expect(config.engine.watch.patterns).toEqual(['custom/*.md']);
            expect(config.engine.watch.patterns).not.toContain('**/*.md');
        });
