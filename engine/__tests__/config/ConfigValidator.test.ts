...
describe('ConfigValidator', () => {
    ...
    describe('validate', () => {
        ...
        describe('engine.watch validation', () => {
            it('should validate patterns array', () => {
                const config = {
                    ...DEFAULT_SPARK_CONFIG,
                    engine: {
                        ...DEFAULT_SPARK_CONFIG.engine,
                        watch: {
                            patterns: ['**/*.md', '**/*.txt'],
                            ignore: ['.git/**'],
                        },
            ...
            it('should throw if engine.watch is missing', () => {
                const config = {
                    ...DEFAULT_SPARK_CONFIG,
                    engine: {
                        debounce_ms: 300,
                ...
                expect(() => validator.validate(config as SparkConfig)).toThrow('engine.watch is required');
            });
            ...
            it('should throw if patterns is not an array', () => {
                ...
                expect(() => validator.validate(config as unknown as SparkConfig)).toThrow(
                    'engine.watch.patterns must be a non-empty array'
                );
            });
            ...
            it('should throw if patterns is empty', () => {
                ...
                expect(() => validator.validate(config)).toThrow('engine.watch.patterns must be a non-empty array');
            });
            ...
            it('should throw if ignore is not an array', () => {
                ...
                expect(() => validator.validate(config as unknown as SparkConfig)).toThrow(SparkError);
                expect(() => validator.validate(config as unknown as SparkConfig)).toThrow(
                    'engine.watch.ignore must be an array'
                );
            });
        });

        describe('engine.debounce_ms validation', () => {
            ...
            it('should throw if debounce_ms is missing', () => {
                const config = {
                    ...DEFAULT_SPARK_CONFIG,
                    engine: {
                        watch: DEFAULT_SPARK_CONFIG.engine.watch,
                    },
                };

                expect(() => validator.validate(config as unknown as SparkConfig)).toThrow(SparkError);
                expect(() => validator.validate(config as unknown as SparkConfig)).toThrow(
                    'engine.debounce_ms must be a number'
                );
            });
