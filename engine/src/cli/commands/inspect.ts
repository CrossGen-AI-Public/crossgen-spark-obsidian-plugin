/**
 * Inspect Command
 * Inspect engine configuration and state
 */
...
function printConfigInfo(config: Awaited<ReturnType<ConfigLoader['load']>>): void {
  ...
  print(`  Debounce: ${config.engine.debounce_ms}ms`);
  print('');
}

function printWatchInfo(config: Awaited<ReturnType<ConfigLoader['load']>>): void {
  print('👁️  Watch Patterns:');
  config.engine.watch.patterns.forEach((pattern) => {
    print(`  + ${pattern}`);
  });
  print('');

  print('🚫 Ignore Patterns:');
  config.engine.watch.ignore.forEach((pattern) => {
    print(`  - ${pattern}`);
  });
  print('');
}
...
export function registerInspectCommand(program: Command): void {
  program
    .command('inspect')
    .description('Inspect engine configuration and state')
